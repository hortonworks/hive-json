...
public class TestJsonSchemaFinder {
    ...
    @Test
    public void testFieldNameValidate() throws IOException, URISyntaxException {
        assertEquals(JsonSchemaFinder.fieldNameValidate("ADD"), "ADD_");
        assertEquals(JsonSchemaFinder.fieldNameValidate("floor"), "floor_");
    }
    ...
    public void testStructs() throws Exception {
        assertEquals("struct<>", JsonSchemaFinder.pickType(new JsonObject()).toString());
        JsonObject struct = new JsonObject();
        struct.addProperty("bool", true);
        assertEquals("struct<bool:boolean>", JsonSchemaFinder.pickType(struct).toString());
        struct = new JsonObject();
        struct.addProperty("str", "value");
        struct.addProperty("i", new LazilyParsedNumber("124567"));
        assertEquals("struct<i:int,str:string>", JsonSchemaFinder.pickType(struct).toString());
        JsonObject nestedStruct = new JsonObject();
        nestedStruct.addProperty("rating", new LazilyParsedNumber("3.25"));
        nestedStruct.addProperty("comments", "Excellent");
        struct.add("description", nestedStruct);
        assertEquals("struct<description:struct<comments:string,rating:decimal(3,2)>,i:int,str:string>", JsonSchemaFinder.pickType(struct).toString());
    }
